...
export default function PromotionFormModal({
  ...
  useEffect(() => {
    ...
    const productosEditados = editing.products.map((p: any) => {
      ...
      return {
        ...
        nombre_producto:
          encontrado?.nombre_producto ?? "Producto no disponible",
      };
    });
  ...
  const handleChange = (
    ...
    setForm((prev) => {
      ...
      if (name === "branch_id") {
        ...
        //  Limpiar carrito al cambiar sucursal
        newForm.products = [];
  ...
  const handleProductsChange = (newProducts: any[], valorNum?: number) => {
    ...
    // NO marcamos manual aquí, eso rompe la división
    let updatedProducts = newProducts.map((p: any) => ({
      product_id: p.product_id ?? p.id,
      nombre_producto: p.nombre ?? p.nombre_producto ?? "",
      cantidad: p.cantidad ?? 1,
      descuento: p.descuento ?? 0,
      manual: p.manual ?? false, // solo si viene marcado manual del usuario
    }));

    if (form.tipo === "combo") {
      // Contar únicamente productos NO manuales
      const nonManualCount = updatedProducts.filter((p) => !p.manual).length;

      // Si todos son manuales → no dividir nada
      if (nonManualCount > 0) {
        const perProduct = valor / nonManualCount;
      ...
      <div>
        ...
        >
          ...
          {branches
            .filter((b) =>
              form.business_id
                ? b.business.nombre_comercial === form.business_id
                : true
            )
            .map((b) => (
