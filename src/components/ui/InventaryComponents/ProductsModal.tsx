            ...
            <label className="font-semibold w-full">
              ...
              <input
                ...
                onChange={(e) =>
                  setFormProducto((f) => ({
                    ...f,
                    nombre_producto: e.target.value.replace(/[0-9]/g, ""),
                  }))
                }
          ...
          <div className="flex flex-col gap-4">
            ...
            <label className="font-semibold w-full">
              ...
              <select
                ...
                onChange={(e) =>
                  setFormProducto((f) => ({ ...f, unit_id: e.target.value }))
                }
                className="w-full border rounded-lg px-3 py-2"
                required
              >
            ...
            <label className="font-semibold w-full">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setFormProducto((f) => ({ ...f, bodega_id: bodegaId }));

                  //  Buscar la bodega seleccionada
                  const bodegaSeleccionada = warehouses.find(
                    (w) => String(w.bodega_id) === String(bodegaId)
                  );

                  //  Extraer el negocio relacionado (branch.business)
                  const negocio =
                    bodegaSeleccionada?.branch?.business?.nombre_comercial ||
                    "";

                  setBusinessName(negocio);
                }}
                className="w-full border rounded-lg px-3 py-2"
                required
              >
                ...
                {warehouses.map((w) => {
                  ...
                  const label =
                    w.codigo ?? w.nombre ?? `Bodega ${w.bodega_id ?? ""}`;
                  return (
            ...
            {/* Negocio relacionado (solo lectura) */}
            <label className="font-semibold w-full">
              Negocio relacionado
              <input
                type="text"
                value={businessName}
                readOnly
                placeholder="Negocio vinculado a la bodega"
